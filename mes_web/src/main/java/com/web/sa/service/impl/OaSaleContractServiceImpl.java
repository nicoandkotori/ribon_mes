...
//    @Transactional(rollbackFor = Exception.class)
    ...
    {
        //同步子表信息--已经存在异常的
        syncSaleContractEdit(1);
        //同步子表信息--不存在异常的
        syncSaleContractEdit(0);
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public ResponseResult syncSaleContractMain() {
...
//                            row++;
    ...
    @Transactional(rollbackFor = Exception.class)
    public ResponseResult syncSaleContract(Integer errState) {
        ResponseResult result = new ResponseResult();
        int row=1;
        OaSaleContract syncData=null;
        int izError=0;
        String errorMsg="";
        try{

            //同步订单子件数据
            List<OaSaleContract> listSync=oaSaleContractMapper.getListBySync(new OaSaleContract(),ParamUtil.getParam("oaDatabase").toString(),errState);
            if(listSync!=null)
            {
                //根据id去重
...
//                            BigDecimal qtySum = listContrast.stream().filter(p->  CustomStringUtils.isNotBlank(p.getField0021())).map(OaSaleContractContrastDetail::getField0021).reduce(BigDecimal.ZERO,BigDecimal::add);
                            ...
                            if(inventoryChild!=null)
                            {
                                ...
                                throw new Exception("子表对应存货数据不存在！");
                            }
...
//                            orderProductionSchedule.setId(String.valueOf(SnowFlakeUtils.getFlowIdInstance().nextId()));
            ...
            if(izError==1&&syncData!=null&&syncData.getId()!=null)
            {
                ...
                oaSaleContractContrastDetailMapper.updateError(syncData.getContrastId(),errorMsg,ParamUtil.getParam("oaDatabase").toString());
            }
        }
        return result;
    }
...
//    @Override
    @Transactional(rollbackFor = Exception.class)
    public ResponseResult syncSaleContractEdit(Integer errState) {
        ResponseResult result = new ResponseResult();
        int row=1;
        OaSaleContract syncData=null;
        int izError=0;
        ...
        try{
            ...
            //同步订单子件数据 编辑
            List<OaSaleContract> listSync=oaSaleContractMapper.getListBySyncEdit(new OaSaleContract(),ParamUtil.getParam("oaDatabase").toString(),errState);
            if(listSync!=null) {
                //查询需要同步的数据
