 ...
 */
@RestController
@RequestMapping(value = "/sysinfo/role")
public class RoleController {
    ...
    public List<Role> getList(HttpServletRequest request,Role query){

        ResponseResult result = new ResponseResult();

        LambdaQueryWrapper<Role> select=new LambdaQueryWrapper<>();
        select = select.eq(Role::getIzDelete, false);
        if(query != null) {
            if (CustomStringUtils.isNotBlank(query.getRoleName())) {
                select = select.like(Role::getRoleName, query.getRoleName());
            }

        }
        select.orderByAsc(Role::getRoleSort);

        List<Role> List = roleService.list(select);
    ...
    public ResponseResult saveOrUpdateRole(String mdatas) throws Exception {
        ...
        List<Role> list = JSON.parseArray(mdatas, Role.class);
    ...
    public TableResult<User> findUserListByRoleid(String roleId) {
        ...
        select = select.apply("id in ( select user_id from sys_role_user where role_id ='"+roleId+"')");
    ...
    public ResponseResult findRoleMenuByRoleid(String roleId) {
        ...
        List<RoleMenu> list = this.roleMenuService.findRoleMenuByRoleId(roleId);
        result.setResult(list);
        return result;
    }
