 ...
 */
@RestController
@RequestMapping(value = "/sysinfo/roleclient")
public class RoleClientController {

    @Autowired
    private IRoleClientService roleService;
    @Autowired
    private IRoleUserClientService roleUserService;
    @Autowired
    private IRoleMenuClientService roleMenuService;
    @Autowired
    ...
    public List<RoleClient> getList(HttpServletRequest request,RoleClient query){

        ResponseResult result = new ResponseResult();

        LambdaQueryWrapper<RoleClient> select=new LambdaQueryWrapper<>();
        select = select.eq(RoleClient::getIzDelete, false);
        if(query != null) {
            if (CustomStringUtils.isNotBlank(query.getRoleName())) {
                select = select.like(RoleClient::getRoleName, query.getRoleName());
            }

        }
        select.orderByAsc(RoleClient::getRoleSort);

        List<RoleClient> List = roleService.list(select);
    ...
    public ResponseResult saveOrUpdateRole(String mdatas) throws Exception {
        ...
        List<RoleClient> list = JSON.parseArray(mdatas, RoleClient.class);
    ...
    public TableResult<User> findUserListByRoleid(String roleId) {
        ...
        select = select.apply("id in ( select user_id from sys_role_user_client where role_id ='"+roleId+"')");
    ...
    public ResponseResult findRoleMenuByRoleid(String roleId) {
        ...
        List<RoleMenuClient> list = this.roleMenuService.findRoleMenuByRoleId(roleId);
        result.setResult(list);
        return result;
    }
