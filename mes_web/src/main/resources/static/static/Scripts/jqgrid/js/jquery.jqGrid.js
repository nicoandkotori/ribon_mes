...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
                ...
                $("input,textarea",cc).bind("focus",function(){this.select();});
				$("input, select, textarea",cc).on("keydown",function(e) {
					...
					// add by yao  2017-7-24  主要实现编辑的时候 向下剪头 直接进入到下一行的编辑状态
					if(e.keyCode===40){
						$($t).jqGrid("downCell",iRow,iCol);  //向下
					}
					// add by yao  2017-7-24  主要实现编辑的时候 向上剪头 直接进入到上一行的编辑状态
					if(e.keyCode===38){
						$($t).jqGrid("upCell",iRow,iCol);  //向上
					}
					e.stopPropagation();
				});
	...
	downCell : function (iRow,iCol) {
		return this.each(function (){
			...
			var totalrows=$($t).jqGrid("getDataIDs").length;
			if(iRow<totalrows){
